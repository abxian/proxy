...
function clashMetaAlpha() {
  ...
  return {
    name: 'shenxianyun-mihomo-alpha',
    targetFile: `shenxianyun-mihomo-alpha-${SIDECAR_HOST}${isWin ? '.exe' : ''}`,
...
function clashMeta() {
  ...
  const urlExt = isWin ? 'zip' : 'gz'
  return {
    name: 'shenxianyun-mihomo',
    targetFile: `shenxianyun-mihomo-${SIDECAR_HOST}${isWin ? '.exe' : ''}`,
    exeFile: `${name}${isWin ? '.exe' : ''}`,
